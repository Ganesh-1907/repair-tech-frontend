...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await apiClient.post('/auth/reset-password', { token, newPassword: form.password });
      setDone(true);
    } catch (err) {
  ...
  if (!token) {
    return (
      <div style={{ minHeight: '100vh', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ background: '#fff', borderRadius: 20, boxShadow: '0 8px 32px rgba(15,23,42,0.10)', padding: '40px 36px', maxWidth: 420, width: '100%', textAlign: 'center' }}>
          <p style={{ color: '#991b1b', fontWeight: 600 }}>Invalid or missing reset token.</p>
          <Link to="/forgot-password" style={{ color: '#4f46e5', fontWeight: 600, fontSize: '0.88rem' }}>Request a new reset link</Link>
        </div>
      </div>
    );
  }
    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: '#fff', borderRadius: 20, boxShadow: '0 8px 32px rgba(15,23,42,0.10)', padding: '40px 36px', width: '100%', maxWidth: 420 }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <p style={{ margin: 0, color: '#64748b', fontSize: '0.88rem' }}>Choose a new password for your account.</p>
        </div>
