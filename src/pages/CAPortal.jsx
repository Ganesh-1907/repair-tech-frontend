    ...
    <div className="ca-portal-page">
      <div className="page-toolbar">
        <div className="header-actions">
          <button className="btn btn-secondary" onClick={() => setNotice('Financial year filter set to FY 2024-25.')}>
            <Calendar size={18} />
            <span>FY 2024-25</span>
          </button>
          <button className="btn btn-primary" onClick={() => setNotice('Audit batch export prepared.')}>
            <Download size={18} />
      ...
      {notice && (
        <div className="success-banner" role="status">
          <span>{notice}</span>
          <button className="icon-btn" onClick={() => setNotice('')} aria-label="Dismiss CA portal message">
            <X size={16} />
          </button>
        </div>
      )}
      ...
      <div className="summary-grid">
        {financialSummary.map((s) => (
          <div key={s.label} className="card summary-card ca-stat">
        ...
        <div className="card report-explorer">
          ...
          <div className="report-cat-grid">
            {reportCategories.map((cat) => (
              <div key={cat.title} className="report-cat-card">
                ...
                <ul className="report-list">
                  {cat.reports.map((report) => (
                    <li key={report} className="report-link">
                      <span>{report}</span>
                      <div className="link-actions">
                        <button className="btn-icon-sm" title="Preview" aria-label={`Preview ${report}`} onClick={() => handleReportAction(report, 'preview')}><FileText size={14} /></button>
                        <button className="btn-icon-sm" title="Download Excel" aria-label={`Download ${report}`} onClick={() => handleReportAction(report, 'download')}><Download size={14} /></button>
                      </div>
                    </li>
                  ))}
                </ul>
        ...
        <div className="side-panels">
          ...
          <div className="card quick-actions-card">
            ...
            <div className="actions-list">
              <button className="action-item" onClick={() => setNotice('Monthly Sales Report download prepared.')}>
              ...
              <button className="action-item" onClick={() => setNotice('Expense Breakdown download prepared.')}>
                <div className="action-info">
                  <PieChart size={18} />
