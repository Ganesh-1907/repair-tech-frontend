...
import {
  ArrowLeft, Plus, Trash2, Mail, Save, Printer, FileText, Download,
} from 'lucide-react';
...
const lbl = { display: 'block', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: '#64748b', marginBottom: 5 };

/* ═══════════════════════════════════════════════════════════════ */
const CampaignJobQuotationPage = () => {
  ...
  useEffect(() => {
    apiClient.get('/records/campaignQuotations')
      .then((res) => {
        const all = Array.isArray(res.data) ? res.data : [];

        const existing = job.id ? all.find((q) => q.jobId === job.id) : null;
        if (existing) {
        ...
        apiClient.get('/records/leadQuotations').then((r2) => {
          const all2 = Array.isArray(r2.data) ? r2.data : [];
          const nums2 = all2.map((q) => { const m = (q.quoteNo || '').match(/^RPB(\d+)$/); return m ? parseInt(m[1], 10) : 0; }).filter((n) => n >= 100001);
          const combined = [...nums, ...nums2];
          const next = combined.length > 0 ? Math.max(...combined) + 1 : 100001;
  ...
  const customerPhone = job.phoneNumber || job.phone || '';
  const customerEmail = job.email || job.customerEmail || '';
  const deviceInfo = [job.deviceType || job.device, job.deviceModel].filter(Boolean).join(' — ');
  const ticketId = job.ticketId || job.id || '';

  const handleEmail = async () => {
    if (emailSending) return;
    if (!customerEmail) { showStatus('No email on this job record. Please update the job with a customer email first.', false); return; }
    setEmailSending(true);
    try {
      let pdfBase64 = null;
      if (printRef.current) pdfBase64 = await generatePdfBase64(printRef.current, `Quotation-${quote.quoteNo}.pdf`);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const payload = {
      jobId: job.id, ticketId, customerName, customerEmail,
      ...
      terms: quote.terms, status: 'Draft', deviceInfo,
    };
    try {
      if (savedId) {
        await apiClient.put(`/records/campaignQuotations/${savedId}`, payload);
      } else {
        const res = await apiClient.post('/records/campaignQuotations', { ...payload, createdAt: new Date().toISOString() });
  ...
  const handleDownload = async () => {
    ...
    setDownloading(true);
    try {
      const b64 = await generatePdfBase64(printRef.current, `Quotation-${quote.quoteNo}.pdf`);
      const a = document.createElement('a');
      a.href = `data:application/pdf;base64,${b64}`;
      a.download = `Quotation-${quote.quoteNo}.pdf`;
    ...
    <div style={{ padding: '0 0 40px', minHeight: '100vh', background: 'var(--bg-page, #f1f5f9)' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <div style={{ fontWeight: 800, fontSize: '1rem', color: 'var(--text-primary, #0f172a)', lineHeight: 1.2 }}>Create Quotation</div>
              <div style={{ fontSize: '0.76rem', color: 'var(--text-muted, #64748b)', marginTop: 1 }}>
                {customerName || 'Job'} · {ticketId}
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button type="button" className="btn btn-secondary" onClick={handleEmail} disabled={emailSending} style={{ gap: 7 }}>
          ...
          <button type="button" className="btn btn-secondary" onClick={handleDownload} disabled={downloading} style={{ gap: 7 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, padding: '20px 28px', alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* Estimate Settings */}
          <div style={card}>
            <div style={cardHead}><span style={cardTitle}>Estimate Settings</span></div>
            ...
            {items.map((item, idx) => (
              <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '32px 1fr 56px 64px 92px 72px 82px 32px', gap: 0, padding: '8px 14px', borderBottom: '1px solid #f1f5f9', alignItems: 'center', background: idx % 2 === 0 ? '#fff' : '#f8fafc' }}>
                ...
                <div style={{ textAlign: 'center' }}>
                  <button className="icon-btn" style={{ width: 26, height: 26, color: items.length === 1 ? '#cbd5e1' : '#ef4444' }} onClick={() => delItem(item.id)} disabled={items.length === 1}>
                    <Trash2 size={12} />
                  </button>
                </div>
              </div>
          ...
          {/* Terms */}
          <div style={card}>
            <div style={cardHead}><span style={cardTitle}>Terms &amp; Conditions</span></div>
            <div style={{ padding: '14px 18px' }}>
              <textarea
                style={{ ...inp, resize: 'none', overflow: 'hidden', minHeight: 80, lineHeight: 1.7, fontSize: '0.84rem' }}
                value={quote.terms}
                onChange={(e) => { setQ('terms', e.target.value); e.target.style.height = 'auto'; e.target.style.height = e.target.scrollHeight + 'px'; }}
            ...
            <div ref={printRef} style={{
                  ...
                  <tbody>
                    {items.map((item, idx) => (
                      <tr key={item.id}>
                        ...
                        <td style={{ padding: '6px 8px', border: '1px solid #dde3ed' }}><strong>{item.name || '—'}</strong></td>
                    ...
                    <tr className="tr-total" style={{ background: '#f8fafc', borderTop: '2px solid #1e293b' }}>
                      <td colSpan={2} style={{ padding: '7px 8px', border: '1px solid #dde3ed', fontWeight: 800 }}>
                        Total &nbsp;<span style={{ fontWeight: 400, color: '#64748b', fontSize: 10.5 }}>({totalQty} items)</span>
                      </td>
              ...
              {quote.terms && (
                <div className="lq-section" style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ fontSize: 11, color: '#475569', lineHeight: 1.7, whiteSpace: 'pre-line' }}>{quote.terms}</div>
                </div>
              )}
              ...
              <div className="lq-sigs" style={{ display: 'flex', justifyContent: 'space-between', marginTop: 48, gap: 24 }}>
                <div className="lq-sig" style={{ flex: 1, textAlign: 'center' }}>
                  ...
                  <div className="lq-sig-line" style={{ borderTop: '1px solid #334155', paddingTop: 6, fontSize: 11, fontWeight: 700 }}>Authorized Signatory (Provider)</div>
                </div>
                <div className="lq-sig" style={{ flex: 1, textAlign: 'center' }}>
                  <p className="lq-sig-name" style={{ fontSize: 10, color: '#64748b', margin: '0 0 14px' }}>{customerName || '—'}</p>
                  <div className="lq-sig-line" style={{ borderTop: '1px solid #334155', paddingTop: 6, fontSize: 11, fontWeight: 700 }}>Authorized Signatory (Client)</div>
                </div>
              </div>
              ...
              <div className="lq-footer" style={{ marginTop: 32, paddingTop: 12, borderTop: '1px solid #f1f5f9', textAlign: 'center', color: '#94a3b8', fontSize: 10 }}>
                Generated by RepairBoy Enterprise — Campaign Jobs Management
              </div>
            </div>
          </div>
        </div>
...
export default CampaignJobQuotationPage;
