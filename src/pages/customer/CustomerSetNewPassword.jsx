...
const CustomerSetNewPassword = () => {
  const { user, customerSetNewPassword, logout } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const result = await customerSetNewPassword(form.password);
    setLoading(false);
    if (result.success) {
      ...
      <div style={{ background: '#fff', borderRadius: 20, boxShadow: '0 8px 32px rgba(79,70,229,0.10)', padding: '40px 36px', width: '100%', maxWidth: 420 }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <p style={{ margin: 0, color: '#64748b', fontSize: '0.88rem' }}>
            Welcome, <strong>{user?.customerName || 'there'}</strong>! Create a secure password to access your portal.
          </p>
        </div>
...
export default CustomerSetNewPassword;
