...
const CustomerForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await apiClient.post('/auth/customer/forgot-password', { email: email.trim().toLowerCase() });
      setSent(true);
    } catch (err) {
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #ede9fe 0%, #f8fafc 60%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: '#fff', borderRadius: 20, boxShadow: '0 8px 32px rgba(79,70,229,0.10)', padding: '40px 36px', width: '100%', maxWidth: 420 }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <p style={{ margin: 0, color: '#64748b', fontSize: '0.88rem' }}>Enter your portal email to receive a reset link.</p>
        </div>
        ...
        {sent ? (
          <div style={{ background: '#dcfce7', border: '1px solid #bbf7d0', borderRadius: 12, padding: '18px 20px', textAlign: 'center' }}>
            ...
            <div style={{ color: '#166534', fontSize: '0.85rem' }}>
              If a customer account exists for <strong>{email}</strong>, a password reset link has been sent.
            </div>
          </div>
        ) : (
